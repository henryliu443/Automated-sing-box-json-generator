...
def _build_nft_ruleset(
    ...
    lines = [
        f"# singbox-guard: OS-level network hardening (generated by automated-sing-box-generator)",
        f"# Default ACCEPT policy — targeted hardening rules only.",
        ...
        f"",
        f"        # Drop INVALID state packets",
        f"        ct state invalid drop",
        f"",
        f"        # --- TCP scan protection ---",
        f"        # Drop NULL scan (no TCP flags)",
        f"        tcp flags == 0x0 drop",
        f"",
        f"        # Drop XMAS scan (FIN+PSH+URG)",
        f"        tcp flags & (fin|psh|urg) == (fin|psh|urg) drop",
        f"",
        f"        # Drop SYN+FIN (invalid combination)",
        f"        tcp flags & (syn|fin) == (syn|fin) drop",
        f"",
        f"        # Drop SYN+RST (invalid combination)",
        f"        tcp flags & (syn|rst) == (syn|rst) drop",
        f"",
        f"        # --- ICMP fingerprint reduction ---",
        f"        # Drop ICMP timestamp requests (OS fingerprinting)",
        f"        ip protocol icmp icmp type timestamp-request drop",
        f"        ip6 nexthdr icmpv6 icmpv6 type 139 drop",
        f"",
        f"        # --- SSH rate limiting ---",
        f"        # Conservative: {ssh_ratelimit_burst} new connections per {ssh_ratelimit_interval}s per source IP.",
        f"        # No IP whitelist — users connect from home/school/mobile/VPN/abroad.",
    ...
    lines += [
        ...
        f"        type filter hook forward priority filter - 10; policy accept;",
        f"        # Forward chain: allow all (proxy outbound traffic must pass through)",
        f"    }}",
        f"",
        f"}}",
        f"",
